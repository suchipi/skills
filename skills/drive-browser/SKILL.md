...
- **Lifecycle - prefer `down`, not `destroy`**: `down` closes the browser but keeps the profile dir, so cookies and logins survive and the next `up` resumes signed in. Tabs do not: `up` always hands you exactly one, showing `--url` or `about:blank`. The browser restores the previous run's tabs from the profile, and the daemon closes them, so a leftover `about:blank` can never turn out to be the tab you are driving. `destroy` deletes that profile - and refuses if you pointed `--profile` at a directory outside the work dir, since that is probably a profile you use elsewhere; `--force` overrides. The puppeteer install in the work dir survives both.
