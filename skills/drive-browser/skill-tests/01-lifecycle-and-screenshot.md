...
**Validates:** `up` launches a browser that outlives the CLI invocation, `screenshot` writes a PNG to the work dir, `status` reports the session, and `down` closes it.
...
- Deterministic:
  ...
  ```
  The first `status` prints `browser: running (headless 1024x768 ...)` and one tab whose URL is the fixture. After `down`, `status` prints `browser: not running` and `profile: ... (preserved)`.
- Visual: **Read** `.tmp/drive-browser-01/01-shot.png` - expect the "Test Page" heading, a text input, and the Greet / Throw / Alert / Open tab buttons.
