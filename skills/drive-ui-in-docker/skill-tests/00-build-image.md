...
**Validates:** the multi-stage Dockerfile builds, and the image has the Node
runtime (copied from the official `node:20` image), suchibot, and the
capture/resize tools. Run this first.
...
Expected: node reports v20, npm v10, `NODE_PATH=/usr/local/lib/node_modules`, and
every binary resolves to a path (no "command not found"). suchibot itself is
proven end-to-end in [02-drive-with-suchibot](02-drive-with-suchibot.md).
